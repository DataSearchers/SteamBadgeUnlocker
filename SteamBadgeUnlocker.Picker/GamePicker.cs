...
using static SteamBadgeUnlocker.Picker.InvariantShorthand;
using APITypes = SteamBadgeUnlocker.API.Types;
...
namespace SteamBadgeUnlocker.Picker
{
    internal partial class GamePicker : Form
    {
        ...
        private void OnActivateGame(object sender, EventArgs e)
        {
            ...
            try
            {
                Process.Start("SteamBadgeUnlocker.Game.exe", info.Id.ToString(CultureInfo.InvariantCulture));
            }
            catch (Win32Exception)
            {
                MessageBox.Show(
                    this,
                    "Failed to start SteamBadgeUnlocker.Game.exe.",
                    "Error",
